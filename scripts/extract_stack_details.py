...
def extract_ram_cores_storage_details(prom_con_obj,start_timestamp):
    ...
    memory_result = execute_point_prometheus_query(prom_con_obj,start_timestamp,total_memory_capacity_query)
    ...
    cpu_result = execute_point_prometheus_query(prom_con_obj,start_timestamp,total_cpu_capacity_query)
    for line in cpu_result:
        node_name = line["metric"]["host_name"]
        final_result[node_name]['cores']=float(line["metric"]["cpu_processor"]) + 1 

    kafka_disk_space_result=execute_point_prometheus_query(prom_con_obj,start_timestamp,f"sum(kafka_disk_volume_size/{1024**3}) by (host_name)")
    for line in kafka_disk_space_result:
        node_name = line["metric"]["host_name"]
        final_result[node_name]['kafka_disk_space_configured_in_TB']=float(line["value"][1])

    hdfs_disk_space_result=execute_point_prometheus_query(prom_con_obj,start_timestamp,f"sort(sum(uptycs_hdfs_node_config_capacity/{(1024**4)}) by (hdfsdatanode))")
    ...
    partition_bytes_used=execute_point_prometheus_query(prom_con_obj,start_timestamp,f"sum(uptycs_used_disk_bytes) by (host_name,partition)")
    for line in partition_bytes_used:
        node_name = line["metric"]["host_name"]
        bytes_used_dict[node_name][line["metric"]["partition"]]=float(line["value"][1])

    partition_percentage_used=execute_point_prometheus_query(prom_con_obj,start_timestamp,f"sum(uptycs_percentage_used) by (host_name,partition)")
...
if __name__=='__main__':
    ...
    extract_ram_cores_storage_details(configuration('longevity_nodes.json') , start_timestamp)
